...
func (r *Registry) PushOciImageByPath(imagePath string) (string, error) {
	...
	body, _ := ioutil.ReadFile(tmp + "/index.json")
	err = json.Unmarshal(body, &jsonMap)
	if err != nil {
		return "", err
	}
	root := strings.TrimPrefix(jsonMap.Manifests[0].Digest, "sha256:")
	if err := copyFile(tmp+"/blobs/sha256/"+root, workdir+"/manifests/latest-v2"); err != nil {
		return "", err
	}
	fileInfo, err := ioutil.ReadDir(tmp+"/blobs/sha256")
	if err != nil {
		panic(err)
	}
	for _, file := range fileInfo {
		if err := copyFile(tmp+"/blobs/sha256/" + file.Name(), workdir+"/blobs/sha256:" +file.Name()); err != nil {
			return "", err
		}
	}
	r.Debugf("[registry] root dir: %s", workdir)
	imageIpfsHash, err := r.uploadDir(directory)
	if err != nil {
		return "", err
	}
