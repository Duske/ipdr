...
func extractManifestPathFromOCIIndex(indexPath string) (string, error) {
	jsonMap := AutoGenerated{
		SchemaVersion: 0,
		Manifests:     nil,
	}
	...
	maniFestHash := strings.TrimPrefix(jsonMap.Manifests[0].Digest, "sha256:")
	return maniFestHash, nil
}

// PushImageByID uploads Docker image by image ID, which is hash or repo tag, to IPFS
func (r *Registry) PushOciImageByPath(imagePath string) (string, error) {
	tmp, err := mktmp()
	if err != nil {
		return "", err
	}
	...
	// Create workspace
	var originalBlobsDir = tmp + "/blobs/sha256/"
	var ociIndexFile = tmp + "/index.json"

	root, err := mktmp()
	workdir := root + "/default"
	mkdir(workdir)
	mkdir(workdir + "/manifests")
	mkdir(workdir + "/blobs")

	// Extract manifest, rename it and move it to new path
	maniFestHash, err := extractManifestPathFromOCIIndex(ociIndexFile)
	if err := copyFile(originalBlobsDir + maniFestHash, workdir+"/manifests/latest-v2"); err != nil {
		return "", err
	}

	// Move blobs to new location
	blobFileInfos, err := ioutil.ReadDir(originalBlobsDir)
	if err != nil {
		panic(err)
	}
	for _, blobFile := range blobFileInfos {
		if err := copyFile(originalBlobsDir + blobFile.Name(), workdir+"/blobs/sha256:" +blobFile.Name()); err != nil {
			return "", err
		}
	}
